...
object MegaProxy {
    ...
    fun start() {
        ...
        try {
            ...
            println("WitAnimeDebug: MegaProxy started on port ${serverSocket?.localPort}")
            Thread {
            ...
            println("WitAnimeDebug: MegaProxy start error: ${e.message}")
        }
    }
    ...
    suspend fun resolve(url: String): String? {
        return try {
            ...
            println("WitAnimeDebug: Mega resolved -> http://127.0.0.1:$port/v/$token.mp4")
            "http://127.0.0.1:$port/v/$token.mp4"
        } catch (e: Exception) {
            println("WitAnimeDebug: Mega resolve error: ${e.message}")
            null
        }
    }
...
class MegaProxyExtractor : com.lagradost.cloudstream3.utils.ExtractorApi() {
    override val name = "MegaProxy"
