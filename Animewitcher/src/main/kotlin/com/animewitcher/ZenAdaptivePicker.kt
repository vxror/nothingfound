...
object ZenAdaptivePicker {
    ...
    private const val PROBE_RANGE_END = "262143"
    ...
    internal fun parseVariants(masterText: String): List<Variant> {
        ...
        while (i < lines.size) {
            ...
            if (line.startsWith("#EXT-X-STREAM-INF") && bandwidth != null) {
                ...
                if (j < lines.size) { 
                    variants.add(Variant(bandwidth, height, lines[j].trim()))
                    i = j 
                }
            }
            i++
    ...
    suspend fun selectBestVariant(masterUrl: String, headers: Map<String, String>): String? {
        return try {
            ...
            val firstSegment = playlistText.lines()
                .firstOrNull { it.isNotBlank() && !it.startsWith("#") }?.trim() ?: return null
