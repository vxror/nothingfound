...
object ZenCryptoAndObfuscation {
    ...
    fun findPackedJsInPage(html: String): Triple<String, List<String>, Int>? {
        for (match in PACKED_JS_SCRIPT_REGEX.findAll(html)) {
            ...
            val snippet = script.substring(start); val endIdx = snippet.indexOf("'.split"); if (endIdx < 0) continue
            val raw = snippet.substring(2, endIdx + 1); val parts = splitPackedJsArgs(raw) ?: continue
    ...
    fun decodePackedJs(payload: String, keywords: List<String>, base: Int): String {
        val parts = keywords.mapIndexedNotNull { i, kw -> if (kw.isNotBlank()) (Regex.escape(toBase(i, base)) to kw) else null }
        if (parts.isEmpty()) return payload
        val alternation = parts.joinToString("|") { it.first }
        val pattern = Regex("\\b(?:$alternation)\\b"); val byEncoded = parts.toMap()
    ...
    private fun splitPackedJsArgs(s: String): List<String>? {
        ...
        while (i < s.length && args.size < 4) {
            if (s[i] == '\'') {
                var end = i + 1
                while (end < s.length) {
                    end = s.indexOf('\'', end); if (end < 0) return null
                    var slashCount = 0; var ci = end - 1
                    while (ci >= 0 && s[ci] == '\\') { slashCount++; ci-- }
                    if (slashCount % 2 == 0) { args.add(s.substring(i + 1, end)); i = end + 1; break }
                    end++
                }
            } else if (s[i] == ',' || s[i] == ' ') { i++ } 
            else { val end = s.indexOfAny(charArrayOf(',', ')', ' '), i).let { if (it < 0) s.length else it }; args.add(s.substring(i, end)); i = end }
        }
