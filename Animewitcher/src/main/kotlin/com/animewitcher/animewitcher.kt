...
class AnimeWitcherProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse? {
        return try {
            ...
            val rating = animeData.optJSONObject("details")?.optString("rating")
            ...
            if (isMovie) {
                ...
                return newTvSeriesLoadResponse(
                    ...
                    episodes = episodes.map { ep ->
                        Episode(
                            data = "$url&ep=${ep.number}",
                            name = ep.name ?: "الحلقة ${ep.number}",
                            episode = ep.number,
    ...
    override suspend fun loadLinks(
        ...
        return try {
            ...
            // 🚀 PARALLEL EXTRACTION from ALL servers
            val allLinks = mutableListOf<ExtractorLink>()
            val lock = Any()
            
            ZenCore.parallelExtract(timeoutMs = 20000L) {
                servers.map { server ->
                    suspend {
    ...
    private suspend fun extractFromServer(
        ...
        try {
            ...
            // 4. 🆕 Heavyweight extractors (25+ hosts including PD Fast, StreamTape, MediaFire)
            ...
            // 6. 🆕 Enhanced Universal sniffer (God-Mode)
